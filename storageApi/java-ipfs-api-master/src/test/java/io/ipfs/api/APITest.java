...
public class APITest {
    ...
    private final IPFS ipfs = new IPFS(new MultiAddress("/ip4/192.168.2.9/tcp/5001"));
    private final Random r = new Random(33550336); // perfect

    @Test
    public void hugeFileStreamTest() throws IOException {
        byte[] hugeData = new byte[1*1024*1024];
        new Random(2).nextBytes(hugeData);
        NamedStreamable.ByteArrayWrapper largeFile = new NamedStreamable.ByteArrayWrapper("massive.txt", hugeData);

        NamedStreamable.FileWrapper file = new NamedStreamable.FileWrapper(new File("/Users/lwy1218/shihj/temp.jpg"));
        MerkleNode addResult = ipfs.add(file).get(0);
//        FileInputStream fis = new FileInputStream("/Users/lwy1218/shihj/temp.jpg");
//        NamedStreamable.StreamWrapper stream = new NamedStreamable.StreamWrapper(fis);
//        MerkleNode addResult = ipfs.add(stream).get(0);
        InputStream in = ipfs.catStream(addResult.hash);
        //Multihash FileHash = Multihash.fromBase58("QmUkBkDGHKP8k9yj8utceATPgMtQKYmQxgrrPBavTo1Rk3");
        //InputStream in = ipfs.catStream(FileHash);
        FileOutputStream outputFileStream = new FileOutputStream("/Users/lwy1218/shihj/123.jpg");;
        ...
        while ((r = in.read(buf)) >= 0) {
            try {
                ...
                outputFileStream.write(buf,0,r);
                offset += r;
