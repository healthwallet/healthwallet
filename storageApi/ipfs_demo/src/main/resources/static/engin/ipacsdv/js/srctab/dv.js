...
;(function($){
    ...
    function dvResize(){
        ...
        //子窗的size
        this.find('.seriesWindow').each(function(i){
            if (i < dvStruct.viewer.nowIndex) {
                $(this).hide();
            }else{
                $(this).show();
    ...
    $.fn.extend({
        "dv":function(options){
            ...
            if(_.isObject(opts.defaultShow)){
                ...
                var len = Math.min(opts.defaultShow.suidArr.length,4);
                for(var i=0;i<len;i++)
            ...
            ////选中与双击  ---  要绑子事件，不然被冒泡阻止了
            for(var i=0;i<opts.viewer.maxCol*opts.viewer.maxRow;i++){
                var id = dvStruct.viewer.winArr[i].winId;var len = dvStruct.viewer.winArr[i].wrappers.length;
                for(var j=0;j<len;j++){
                    var el = dvStruct.viewer.winArr[i].wrappers[j].element;
                    var hammer = new Hammer.Manager(el);
                    var singleTap = new Hammer.Tap({ event: 'singleTap' });
                    var doubleTap = new Hammer.Tap({event: 'doubleTap', taps: 2,interval:600 });
                    hammer.add([singleTap,doubleTap]);
                    doubleTap.recognizeWith(singleTap);
                    hammer.on('singleTap',function(e){
                        $('.seriesWindow').removeClass('checked');
                        if($(e.target).hasClass('seriesWindow')){
                            $(e.target).addClass('checked');
                        }else{
                            $(e.target).parents('.seriesWindow').addClass('checked');
                        }
                        ...
                        //    没懂为什么要两次setAllEle（），又开又关。。。？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？
                        }
                    });
                    hammer.on('doubleTap',function(){
                        if(dvStruct.viewer.row!=1||dvStruct.viewer.col!=1){
                            dvStruct.viewer.oneoneMap();
                        }else{
                            dvStruct.viewer.backToUsedMap();
                        }
                    });
                }
            }
            //定位线
            $('.js-reference').on('touchstart',function(){
              ...
              /*  if($(this).hasClass('leftBtnActive'))
                {
                    $('.btnArea').removeClass('leftBtnActive');
                    $(this).removeClass('leftBtnActive')
                    dvStruct.viewer.btnStates.rectangleRoiTouch.enable=false;
                    dvStruct.viewer.btnStates.wwwcTouchDrag.enable=false;
                    dvStruct.viewer.btnStates.touchDragTool.enable=false;
                    dvStruct.viewer.btnStates.lengthTouch.enable=false;
                    dvStruct.viewer.btnStates.referenceLines.enable=false;
                }
                else
                {
                ...
                if( $(this).hasClass('leftBtnActive'))
                {
                    for(var i=0;i<dvStruct.viewer.winArr.length;i++){
                        ...
                        for(var j=0;j<win.wrappers.length;j++){
                            try{
                                var element = win.wrappers[j].element;
                                setAllEle(function(element){
            ...
            //选中初始的
            $('.seriesWindow').removeClass('checked');
            $('.seriesWindow:first').addClass('checked');

            //上下翻页
            $('.js-paging.prev').on('touchstart',function(){
                dvStruct.fun.prevPage();
            });
            $('.js-paging.next').on('touchstart',function(){
            ...
            $('.js-wwwl').on('touchstart',function(){
                ...
                else
                {
                    ...
                    $(this).addClass('leftBtnActive');
                    dvStruct.viewer.btnStates.rectangleRoiTouch.enable=false;
                    dvStruct.viewer.btnStates.wwwcTouchDrag.enable=true;
                    dvStruct.viewer.btnStates.touchDragTool.enable=false;
                    dvStruct.viewer.btnStates.lengthTouch.enable=false;
               ...
               /* for(var i=0;i<dvStruct.viewer.winArr.length;i++){
                    ...
                    for(var j=0;j<win.wrappers.length;j++){
                        try{
                            var element = win.wrappers[0].element;
                            //cornerstoneTools.panMultiTouch.deactivate(element);
                            cornerstoneTools.rectangleRoiTouch.deactivate(element);dvStruct.viewer.btnStates.rectangleRoiTouch.enable=false;
                            cornerstoneTools.lengthTouch.deactivate(element);dvStruct.viewer.btnStates.lengthTouch.enable=false;
                            cornerstoneTools.touchDragTool.deactivate(element);dvStruct.viewer.btnStates.touchDragTool.enable=false;
                            cornerstoneTools.wwwcTouchDrag.activate(element);dvStruct.viewer.btnStates.wwwcTouchDrag.enable=true;
            ...
            $('.js-pan').on('touchstart',function(){
               /* $('.btnArea').removeClass('leftBtnActive');
                ...
                else
                {
                    ...
                    dvStruct.viewer.btnStates.rectangleRoiTouch.enable=false;
                    dvStruct.viewer.btnStates.wwwcTouchDrag.enable=false;
                    dvStruct.viewer.btnStates.touchDragTool.enable=true;
                    dvStruct.viewer.btnStates.lengthTouch.enable=false;
               ...
               /* for(var i=0;i<dvStruct.viewer.winArr.length;i++){
                    ...
                    for(var j=0;j<win.wrappers.length;j++){
                        try{
                            ...
                            cornerstoneTools.rectangleRoiTouch.deactivate(element);dvStruct.viewer.btnStates.rectangleRoiTouch.enable=false;
                            cornerstoneTools.lengthTouch.deactivate(element);dvStruct.viewer.btnStates.lengthTouch.enable=false;
                            cornerstoneTools.wwwcTouchDrag.deactivate(element);dvStruct.viewer.btnStates.wwwcTouchDrag.enable=false;
                            //cornerstoneTools.panMultiTouch.activate(element);
                            cornerstoneTools.touchDragTool.activate(element);dvStruct.viewer.btnStates.touchDragTool.enable=true;
                        }catch(e){
                            console.error(e);
                        }
                    }
            ...
            //js-length
            $('.js-length').on('touchstart',function(){
                //添加选中和取消选中处理
                if($(this).hasClass('leftBtnActive'))
                {
                    $('.btnArea').removeClass('leftBtnActive');
                ...
                else
                {
                    ...
                    dvStruct.viewer.btnStates.wwwcTouchDrag.enable=false;
                    dvStruct.viewer.btnStates.touchDragTool.enable=false;
                    dvStruct.viewer.btnStates.lengthTouch.enable=true;
                }
               ...
               /* for(var i=0;i<dvStruct.viewer.winArr.length;i++){
                    ...
                    for(var j=0;j<win.wrappers.length;j++){
                        try{
                            ...
                            //cornerstoneTools.panMultiTouch.activate(element);
                            cornerstoneTools.touchDragTool.deactivate(element);dvStruct.viewer.btnStates.touchDragTool.enable=false;
                            cornerstoneTools.lengthTouch.activate(element);dvStruct.viewer.btnStates.lengthTouch.enable=true;
                        }catch(e){
                            console.error(e);
                        }
                    }
            ...
            //测量矩形
            $('.js-rect').on('touchstart',function(e){
                // $('.btnArea').removeClass('leftBtnActive');
                // $(this).addClass('leftBtnActive');
                //添加选中和取消选中处理
                if($(this).hasClass('leftBtnActive'))
                ...
                else
                {
                    $('.btnArea').removeClass('leftBtnActive');
                    $(this).addClass('leftBtnActive');
                    dvStruct.viewer.btnStates.rectangleRoiTouch.enable=true;
                    dvStruct.viewer.btnStates.wwwcTouchDrag.enable=false;
                    dvStruct.viewer.btnStates.touchDragTool.enable=false;
                    dvStruct.viewer.btnStates.lengthTouch.enable=false;
                }
              ...
              /*  for(var i=0;i<dvStruct.viewer.winArr.length;i++){
                    ...
                    for(var j=0;j<win.wrappers.length;j++){
                        try{
                            ...
                            cornerstoneTools.touchDragTool.deactivate(element);dvStruct.viewer.btnStates.touchDragTool.enable=false;
                            cornerstoneTools.lengthTouch.deactivate(element);dvStruct.viewer.btnStates.lengthTouch.enable=false;
                            cornerstoneTools.rectangleRoiTouch.activate(element);dvStruct.viewer.btnStates.rectangleRoiTouch.enable=true;
                        }catch(e){
                            console.error(e);
                        }
                    }
            ...
            //清空
            $('.js-clearToolState').on('touchstart',function() {
            ...
            //停止
            $('.js-stop').click(function(){$(this).hide();
                $('.js-play').show();
                dvStruct.fun.stop();
            ...
            //打开关闭图列
            $(this).find('.js-hideGallery').click(function(){
                $(dvStruct.container).find('.leftGallery').hide(500);
                $(_this).find('.js-openGallery').css('margin-right',0);
            });
            $(this).find('.js-openGallery').click(function(){
                $(dvStruct.container).find('.leftGallery').show(500);
                $(_this).find('.js-openGallery').css('margin-right','80px');
            });
            ...
            //信息文字
            $('.js-infoVisible').on('touchstart',function(){
                if($(this).hasClass('checked')){
                    $(this).removeClass('checked');
                    $('.js-info').hide();
                }else{
